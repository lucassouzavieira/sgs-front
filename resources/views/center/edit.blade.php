...
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3>{{ trans('general.edit') }} - {{trans('general.center')}}</h3>
        </div>
        <!-- /.box-header -->
        <!-- form start -->
        <form role="form" method="post" action="{{ route('center.update', ['id' => $obj->id]) }}">
            <div class="box-body">
                {{ csrf_field() }}
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" name="description" placeholder="{{ trans('general.description') }}"
                           required type="text" min="5" value="{{ $obj->description }}">
                </div>
            </div>
            <!-- /.box-body -->
